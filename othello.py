...
def nextgame(game, row, clm, Player):
    if game[row][clm] == 0:
        for d_iter in range(9):
            ...
            while True:
                length += 1
                if row + dr * length < 0 or row + dr * length > 7 or clm + dc * length < 0 or clm + dc * length > 7:
                    break
...
class gameUI(tk.Frame):
    ...
    def cpu_turn(self):
        able = placable(self.game, -1)
        if numpy.sum(able) > 0:
            row, clm = random.choice([(r, c) for r in range(8) for c in range(8) if able[r][c] != 0])
            self.nextstatus = self.game.copy()
            nextgame(self.nextstatus, row, clm, -1)
            self.game = self.nextstatus
            self.player = 1
            self.on_draw()
            if numpy.sum(placable(self.game, 1)) == 0:
                self.player = -1
                if numpy.sum(placable(self.game, -1)) == 0:
                    self.game_end()
